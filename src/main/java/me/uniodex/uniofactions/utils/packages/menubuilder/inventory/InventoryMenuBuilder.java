...
import me.uniodex.uniofactions.utils.packages.menubuilder.MenuBuilder;
...
@SuppressWarnings({"unused", "UnusedReturnValue", "CaughtExceptionImmediatelyRethrown"})
public class InventoryMenuBuilder
    ...
    private InventoryListener inventoryListener = UnioFactions.getInstance().getInventoryListener();
    ...
    public InventoryMenuBuilder withEventHandler(InventoryEventHandler eventHandler) {
        try {
            inventoryListener.registerEventHandler(this, eventHandler);
    ...
    public InventoryMenuBuilder onInteract(InventoryMenuListener listener, ClickType... actions) {
        ...
        try {
            inventoryListener.registerListener(this, listener, actions);
        } catch (IllegalArgumentException e) {
            throw e;
        }
        return this;
    ...
    public void dispose() {
        inventoryListener.unregisterAllListeners(getInventory());
    }

    public void unregisterListener(InventoryMenuListener listener) {
        try {
            inventoryListener.registerListener(this, listener, ALL_CLICK_TYPES);
        } catch (IllegalArgumentException e) {
            throw e;
        }
    }
